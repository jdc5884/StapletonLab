...
plot(ntyped(height), ylab="No. typed markers", main="No. genotypes by individual")
plot(ntyped(height, "mar"), ylab="No. typed individuals", main="No. genotypes by marker")
dev.off()

height <- drop.nullmarkers(height)

#scan with variance
heightprob <- calc.genoprob(height, step=1, error.prob=0.01)

heightv <- scanonevar(cross = height, #issues with this function, maybe the sample is too small?
                    pheno.col = 1)
#######using scanonevar with vqtl package syntax########
#heightv = scanonevar(cross = height,
